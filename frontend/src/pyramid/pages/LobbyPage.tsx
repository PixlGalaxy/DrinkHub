...
import { Users, Plus, LogIn, Layers, ChevronRight, ArrowLeft, RotateCcw } from 'lucide-react';
...
export function LobbyPage() {
  ...
  useEffect(() => {
    ...
    if (state?.roomNotFound) {
      ...
      navigate('/pyramid', { replace: true, state: {} });
    }
  ...
  return (
    <div className="min-h-svh flex flex-col bg-[#0a0a0f] bg-grid">
      {toastMsg && (
        <Toast message={toastMsg} type="error" onClose={() => setToastMsg('')} autoCloseDuration={5000} />
      )}
      <GameNavbar />

      <main className="flex-1 w-full max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12 lg:py-16 flex flex-col">

        {/* Hero */}
        <div className="text-center mb-6 sm:mb-10 lg:mb-12 fade-up">
          <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 rounded-3xl
                          bg-gradient-to-br from-purple-600 to-violet-800
                          mx-auto mb-4 sm:mb-6 flex items-center justify-center
                          shadow-2xl shadow-purple-600/30 float">
            <Layers size={40} className="text-white sm:hidden" strokeWidth={1.5} />
            <Layers size={48} className="text-white hidden sm:block lg:hidden" strokeWidth={1.5} />
            <Layers size={56} className="text-white hidden lg:block" strokeWidth={1.5} />
          </div>
          <h1 className="text-white font-black tracking-tight mb-3 text-balance
                         text-3xl sm:text-4xl lg:text-5xl">
            Pyramid
          </h1>
          <p className="text-white/50 text-sm sm:text-base lg:text-lg leading-relaxed max-w-md mx-auto text-balance">
            {lang === 'es'
              ? 'Farolea, reclama cartas de la pirámide y asigna bebidas — o prepárate para beber el doble.'
              : 'Bluff your way through the pyramid, claim cards and assign drinks — or be ready to drink double.'}
          </p>
        </div>
        ...
        {storedSession && (
          <div className="bg-purple-900/20 border border-purple-500/30 rounded-2xl p-4
            ...
            <button
              ...
              className="shrink-0 flex items-center gap-2 bg-purple-600 hover:bg-purple-500
                         text-white font-bold px-4 py-2 rounded-xl text-sm transition-all active:scale-95"
            >
        ...
        <div className="bg-[#13131a] border border-white/8 rounded-2xl sm:rounded-3xl
          ...
          {mode === 'select' && (
            <div className="flex flex-col gap-3">
              <p className="text-center text-white/40 text-sm mb-2">{t('lobby.subtitle', lang)}</p>

              <button
                onClick={() => setMode('create')}
                className="w-full flex items-center justify-between gap-3
                           bg-purple-600 hover:bg-purple-500 active:scale-[0.98]
                           text-white font-bold px-5 sm:px-6 py-4 sm:py-5
                           rounded-xl sm:rounded-2xl transition-all shadow-lg shadow-purple-600/25"
              >
          ...
          {(mode === 'create' || mode === 'join') && (
            <div className="flex flex-col gap-4 sm:gap-5">
              <button
                ...
                className="flex items-center gap-2 text-white/40 hover:text-white/80 text-sm transition-colors -mb-1 self-start"
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border-2 border-white/10 rounded-xl
                               ...
                               placeholder-white/15 focus:outline-none focus:border-purple-500/60
                               focus:bg-white/8 transition-all text-center tracking-[0.25em]"
                  />
                </div>
              )}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2
                           bg-purple-600 hover:bg-purple-500 active:scale-[0.98]
                           text-white font-bold px-5 py-4 sm:py-5 rounded-xl text-base sm:text-lg
                           transition-all shadow-lg shadow-purple-600/25 mt-1"
              >
                {mode === 'create' ? <Plus size={18} strokeWidth={2.5} /> : <Users size={18} strokeWidth={2} />}
        ...
        <div className="mt-6 sm:mt-10 fade-up">
          <div className="grid grid-cols-4 gap-2 sm:gap-3">
            {[
              { value: '52', label: lang === 'es' ? 'Cartas' : 'Cards', color: 'text-purple-400' },
              { value: '10', label: lang === 'es' ? 'Pirámide' : 'Pyramid', color: 'text-violet-400' },
              { value: '3', label: lang === 'es' ? 'Tu mano' : 'Your hand', color: 'text-indigo-400' },
              { value: '×4', label: lang === 'es' ? 'Máx. bebidas' : 'Max drinks', color: 'text-red-400' },
            ].map(stat => (
              <div key={stat.label}
                   className="bg-white/4 border border-white/5 rounded-xl px-2 sm:px-3 py-3 sm:py-4 text-center">
